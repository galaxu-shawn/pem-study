...
center_of_eval_plane = [0, 0, 0] # center of the grid
offset_x = 80 # in meter, + and - from center_of_eval_plane
offset_y = 60

# grid bounds where heatmap will be calculated [xmin,xmax,ymin,ymax]
grid_bounds = [center_of_eval_plane[0]-offset_x, center_of_eval_plane[0]+offset_x,
               center_of_eval_plane[1]-offset_y, center_of_eval_plane[1]+offset_y]
eval_plane_zpos = center_of_eval_plane[2]
...
wl = 3e8 / center_freq
prim_name1a = all_actors.add_actor(name='prim_example', generator=prim, mat_idx=mat_manager.get_index('pec'),
                                 color='green',
...
all_actors.actors[prim_name1a].coord_sys.pos = [20,0,0]
all_actors.actors[prim_name1a].coord_sys.update()

prim_name1b = all_actors.add_actor(name='prim_example', generator=prim, mat_idx=mat_manager.get_index('pec'),
                                 color='green',
                                 target_ray_spacing=wl,dynamic_generator_updates=False) #dynamic_generator_updates=False will not create a new geometry at each time step

all_actors.actors[prim_name1b].coord_sys.pos = [40,0,0]
all_actors.actors[prim_name1b].coord_sys.update()


prim_name2a = all_actors.add_actor(name='prim_example', generator=prim, mat_idx=mat_manager.get_index('glass'),
                                  color='blue',
                                  target_ray_spacing=wl,dynamic_generator_updates=False) #dynamic_generator_updates=False will not create a new geometry at each time step

all_actors.actors[prim_name2a].coord_sys.pos = [-20,0,0]
all_actors.actors[prim_name2a].coord_sys.update()


prim_name2b = all_actors.add_actor(name='prim_example', generator=prim, mat_idx=mat_manager.get_index('glass'),
                                  color='blue',
                                  target_ray_spacing=wl,dynamic_generator_updates=False) #dynamic_generator_updates=False will not create a new geometry at each time step

all_actors.actors[prim_name2b].coord_sys.pos = [-35,0,0]
all_actors.actors[prim_name2b].coord_sys.rot = euler_to_rot(phi=45, theta=0, order='zyz', deg=True)
all_actors.actors[prim_name2b].coord_sys.update()
