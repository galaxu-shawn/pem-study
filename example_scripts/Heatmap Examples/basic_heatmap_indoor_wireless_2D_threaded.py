...
heatmap.start_background_computation(pulse_idx=0, freq_idx=0, tx_idx=0, rx_idx=0)

# While it's running, periodically update visualization
import time
while not heatmap.is_computation_complete():
    # Update mesh with current data
    if heatmap.update_mesh_threaded(function='db'):
        # Visualize or process the current mesh
        pass
    
    # Show progress
    print(f"Progress: {heatmap.get_computation_progress() * 100:.1f}%")
    
    # Wait a bit before next update
    time.sleep(1.0)

# Get final results
# final_mesh = heatmap.get_current_mesh(function='db')
heatmap.mesh.plot()