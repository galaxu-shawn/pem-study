"""
Simple P2P (Communications) Example - Minimal Setup
====================================

Simple scene, 1 perfectly conducting plate target, 1 Tx and 1 Rx located at different location with a pulsed Doppler waveform.

This example demonstrates the most basic p2p simulation workflow:
1. Import utilities
2. Set simulation parameters  
3. Create scene with target and tx/rx
4. Define waveform
5. Add tx and rx antennas
6. Configure simulation
7. Run simulation
8. Retrieve results

This is the simplest possible p2p (antenna to antenna coupling) example to get started.
"""
...
from pem_utilities.antenna_device import Waveform, add_single_tx, add_single_rx, enable_coupling # antenna waveform and sources
...
# 2) Set waveform parameters (simple 60 GHz). This will be used to create a pulsed Doppler waveform.
center_freq = 60e9        # 60 GHz
...
target_name = all_actors.add_actor(filename=os.path.join(paths.models,'large_sheet.stl'),
...
all_actors.actors[target_name].coord_sys.pos = [0, -10, -1]
all_actors.actors[target_name].coord_sys.rot = euler_to_rot(phi=90, theta=0, order='zyz', deg=True)
all_actors.actors[target_name].coord_sys.lin = [0, 0.25, 0] # assign some linear velocity
all_actors.actors[target_name].coord_sys.update() # set the coordinate system of the target actor

# Add tx platform (empty actor to attach tx to)
tx_actor_name = all_actors.add_actor()
all_actors.actors[tx_actor_name].coord_sys.pos = [10, 0, 0]  # Radar at origin
all_actors.actors[tx_actor_name].coord_sys.update()

# Add rx platform (empty actor to attach rx to)
rx_actor_name = all_actors.add_actor()
all_actors.actors[rx_actor_name].coord_sys.pos = [-10, 0, 0]  # Radar at origin
...
# 5) Add tx antenna device and Rx antenna device.
# this utility function creates a antenna device with a single Tx or Rx. This could be an ffd file or a parametric beam
ant_device_tx = add_single_tx(all_actors, # all actors is the scene tree, it is included here as we will append  this radar device to the list of actors
                             waveform, 
                             mode_name,
...
ant_device_rx = add_single_rx(all_actors, # all actors is the scene tree, it is included here as we will append  this radar device to the list of actors
                             waveform, 
                             mode_name,
...
enable_coupling(mode_name, ant_device_tx, ant_device_rx) # enable coupling between Tx and Rx devices, this will allow the simulation to compute the coupling between the Tx and Rx antennas
...
output_format = ant_device_tx.waveforms[mode_name].output
...
for idx in range(100): # lets run 100 iterations, update position of target for each iteration
    ...
    (ret, response) = pem.retrieveP2PResponse(ant_device_tx.modes[mode_name], ant_device_rx.modes[mode_name], RssPy.ResponseType.RANGE_DOPPLER)
    im = 20*np.log10(np.abs(response[0,0])) # index [0][0] corresponds to the first Tx/Rx pair
    modeler.update_frame(plot_data=im,plot_limits=[-120, -90]) # update the visualization frame, including adding a plot of the response data
