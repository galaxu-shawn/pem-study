...
hfss_csv = r'C:\Users\asligar\OneDrive - ANSYS, Inc\Desktop\delete\hfss2.csv'
...
if compare_results:
    ...
    mono_rcs_db_hfss= hfss_data.T[2]
    phi_hfss= hfss_data.T[1]


R_dist = 1e6 # distance from target to radar, should be in far field if we want true RCS
...
# target is going to just sit at the center of teh scene,
actor_target_name = all_actors.add_actor(filename =os.path.join(paths.models, 'explorer_ship_meter.stl'),target_ray_spacing=0.05) # ray_spacing in meters on target
...
# api_core.isOK(api.setPrivateKey("RayShootGrid", "SBR," + str(0.5)))
...
sim_options.field_of_view = 360 # if 180 it will only shoow in the +X direction
...
radius = R_dist # distance away from the target origin
...
for idx in tqdm(range(num_phi_samples)):
    ...
    #only need to update the radar device, everything else is static at center of scene
    # update the radar device position for current position
    all_actors.actors[actor_radar_name].coord_sys.pos = all_points[idx]
    rot = euler_to_rot(phi=phi_points[idx]-180,theta=0,psi=0) # rotate antenna so it is always pointing back at the target
    ...
    # approximate RCS from radar range equation. This is a very simple approximation, but it is a good starting point
    # in 25.1, Perceive EM supports plane wave excitations that will directly provide this value and it won't need to
    # be approximated. The RCS provided here is using the Tx/Rx loss and antenna gain, and assume the target is a point

    Pr = np.abs(response)
    Pr_dB = 20 * np.log10(Pr) # response is s-params
    # TODO get transmit power from API
    Pt = 1  # 1Watt, input power, 0dBw is source power
    Pt_dB = 10 * np.log10(Pt)
    Gt = 2.15  # Gain dipole, in dB
    Gr = 2.15
    rcs_scaled_dB = Pr_dB + 30 * np.log10(4 * np.pi) + 40 * np.log10(R_dist) - (
            Pt_dB + Gt + Gr + 20 * np.log10(3e8 / (center_freq)))  # radar range equation in dB

    all_results_rcs_dB.append(rcs_scaled_dB)
    all_results.append(response) # all_results will be in format (num_phi_points, num_freq_samples)
...
fig, ax = plt.subplots()
ax.plot(phi_points, np.flip(all_results_rcs_dB),label='PEM') # plot the first frequency sample
if compare_results:
    Pr_dB = mono_rcs_db_hfss
    # Pr_dB = 20 * np.log10(Pr)  # response is s-params
    # TODO get transmit power from API
    Pt = 1  # 1Watt, input power, 0dBw is source power
    Pt_dB = 10 * np.log10(Pt)
    Gt = 2.15  # Gain dipole, in dB
    Gr = 2.15
    mono_rcs_db_hfss = Pr_dB + 30 * np.log10(4 * np.pi) + 40 * np.log10(R_dist) - (
            Pt_dB + Gt + Gr + 20 * np.log10(3e8 / (center_freq)))  # radar range equation in dB
