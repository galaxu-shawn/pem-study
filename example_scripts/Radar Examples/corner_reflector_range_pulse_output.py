# This example demonstrates a new output type introduced in Perceive EM 25.2, which is RANGE_PULSE.
...
if float(pem_api_manager.version) < 252:
    raise RuntimeError("This example requires Perceive EM API version 25.2 or later.")
...
waveform_dict = {
    "mode": "PulsedDoppler",
    "output": "RangePulse", # This is the new output type introduced in Perceive EM 25.2
...
pem_api_manager.isOK(pem.activateRangePulseResponse(ant_device.modes[mode_name],
                                                ant_device.range_pixels,
                                                ref_pixel_begin, # range reference pixel
                                                range_start,
                                                ant_device.r_specs))
...
for iFrame in tqdm(range(num_frames), disable=True):
    ...
    (ret, response) = pem.retrieveResponse(ant_device.modes[mode_name], RssPy.ResponseType.RANGE_PULSE)
