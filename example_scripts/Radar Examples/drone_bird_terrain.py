...
time_stamps = np.linspace(0, 10, 100) # 10 seconds, 100 frames

# range of values that lat/lon can be generated around
scene_lat_lon = (45.306901, -121.732871)
...
drone_pos = [-100, 0, 50] # this will be offset from terrain  in Z
...
actor_theta = 0
actor_psi = 0
all_actors.actors[actor_bird_name].rotor_ang = [0, 0, rotor_speed * 2 * np.pi]  # in rad/s
...
ant_device = add_single_tx_rx(all_actors,waveform,mode_name,pos=radar_pos,rot = radar_rot, ffd_file='dipole.ffd',scale_pattern=2)
