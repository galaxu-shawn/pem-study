...
for iFrame in tqdm(range(numFrames), disable=True):
    ...
    # calculates the below values based on the initial pos
    all_actors.actors[actor].coord_sys.pos = [0,0,0]
    all_actors.actors[actor].coord_sys.rot = np.eye(3)
    all_actors.actors[actor].coord_sys.lin = [0,0,0] # m\s
    all_actors.actors[actor].coord_sys.ang  = [0,0,0] # rad\s
    all_actors.actors[actor].coord_sys.update()
