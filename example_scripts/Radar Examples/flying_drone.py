...
actor_terrain_name = all_actors.add_actor(filename=os.path.join(paths.models,'whole-scene-static.stl'),
                                             mat_idx=mat_manager.get_index('asphalt'),
                                             color='grey')
actor_terrain_name2 = all_actors.add_actor(filename=os.path.join(paths.models,'whole-scene-static.stl'),
                                             mat_idx=mat_manager.get_index('asphalt'),
                                             color='grey')
...
actor_vehicle1_name = all_actors.add_actor(name='vehicle1',filename=os.path.join(paths.models,'Audi_A1_2010/Audi_A1_2010.json'),
...
# this velocity update is not based on the linear velocity equation, but instead we will update the position of the
# actor based on a series of points that we will interpolate. This is useful for objects that we don't know the velocity
# by default, use_linear_velocity_equation_update is set to True, which will update the position based on the velocity.
# if it is false, we we estimate velocity based on the position of the actor
...
# animate the body to walk. An argument 'target_ray_spacing' can be used to create an adaptive shoot grid that will
# use a fixed ray spacing, in meters, no matter where the actor is in the scene
actor_pedestrian_name = all_actors.add_actor(filename=os.path.join(paths.models,'Walking_speed50_armspace50.dae'),
...
# all_actors.actors[actor_vehicle1_name].coord_sys.coord_sys.lin = (5., 0., 0.)
all_actors.actors[actor_vehicle1_name].coord_sys.update()
...
all_actors.actors[actor_pedestrian_name].coord_sys.pos = (80., 5, 0.)
...
ant_device = AntennaDevice('example_1tx_1rx.json',
                           parent_h_node=all_actors.actors[actor_quadcopter_name].h_node,
                           all_actors=all_actors)
ant_device.initialize_mode(mode_name='mode1')
ant_device.coord_sys.pos = (.25, 0., 0)
ant_device.coord_sys.rot = euler_to_rot(phi=10, theta=0, order='zyz', deg=True)
...
# If save each ffd as a gltf file, we can use the following code to save each ffd as a gltf file
# for each in ant_device.all_antennas_properties:
#     print(f'Creating GLTF for:{each}')
#     gltf_output_path = os.path.join(paths.output, f"{each}.gltf")
#     convert_ffd_to_gltf(ant_device.all_antennas_properties[each]['mesh'],cmap='magma',output_path=gltf_output_path,show_plot=True)
