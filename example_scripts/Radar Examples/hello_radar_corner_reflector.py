...
def facet_trihed(side, idx_coat=1):
    ...
    fct['name'] = f'Trihedral with isosceles triangle sides: s = {side}'
    # Define triangulation for triangular faces
    fct['tri'] = np.array([
        [0, 1, 2],
        [0, 2, 3],
        [0, 3, 1]
    ...
    fct['nnode'] = fct['xyz'].shape[0]
    fct['nfct'] = fct['tri'].shape[0]
    fct['iatr1'] = np.full(fct['nfct'], idx_coat, dtype=np.int32)

    return fct


def facet_rot(fct, axis, angle):
    """
    Helper function to rotate facet model around specified axis.
    Note: This is a placeholder - actual implementation would need to be provided.

    Parameters:
        fct (dict): Facet model structure
        axis (str): Rotation axis ('x', 'y', or 'z')
        angle (float): Rotation angle in degrees

    Returns:
        dict: Rotated facet model
    """
    # TODO: Implement rotation logic here
    # This would involve creating rotation matrices and applying them to fct['xyz']
    return fct
...
# create corner reflector of length 0.7
facet = facet_trihed(0.7)
hCorner = RssPy.SceneElement()
isOK(api.addSceneElement(hCorner))
# set triangles from mesh
isOK(api.setTriangles(hCorner,facet['xyz'],facet['tri'],0)) # coating of 0 (pec) for all triangles
# isOK(api.setTriangles(hCorner,facet['xyz'],facet['tri'],np.array([0,1,2]))) # a different coating for each triangle
...
cornerCS = CoordSys(None,hCorner)
cornerCS.pos = (2.,0.,1.)
cornerCS.lin = (14.,0.,0.)
cornerCS.rot = np.eye(3)*-1
...
isOK(api.addFocusedRayGroup(cornerCS.hNode, groupRaySpacing))
