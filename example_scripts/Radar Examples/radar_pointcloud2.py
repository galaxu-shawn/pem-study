...
actor_quad_name = all_actors.add_actor(filename=os.path.join(paths.models,'Quadcopter/Quadcopter.json'),scale_mesh=10)
...
all_actors.actors[actor_vehicle1_name].velocity_mag = 0.
...
all_actors.actors[actor_quad_name].coord_sys.pos = (50, 10, 0.)
all_actors.actors[actor_quad_name].velocity_mag = 10.
all_actors.actors[actor_quad_name].coord_sys.rot = euler_to_rot(phi=-0, theta=0, order='zyz', deg=True)
all_actors.actors[actor_quad_name].coord_sys.update()
...
ant_device.coord_sys.pos = (4.1, 0, 1)
...
modeler = ModelVisualization(all_actors,
                             show_antennas=True,
                             rng_domain=np.linspace(0, 100, 512),
                             vel_domain=np.linspace(0, 100, 512),
                             fps=fps,
...
for idx in tqdm(range(num_frames), disable=True):
    ...
    rd_im = 20*np.log10(np.abs(response[0,0]))
    ...
    if len(target_list) > 0:
        ...
        for each in target_list:
            print(f'Target Velocity: {target_list[each]["velocity"]}')
        x = []
        y = []
        all_mag = []
        max_size = 2
        ...
        if time == 0:
            ...
            ax.set_xlim(-600, 600)
            ax.set_ylim(-600, 600)
            h_chart = pv.ChartMPL(f, size=(0.275, 0.375), loc=(0.0, 0.0))
    ...
    modeler.update_frame(plot_data = cfar.T)
modeler.close()
