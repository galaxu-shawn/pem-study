...
rcs = RCS(rcs_mode='monostatic',rayshoot_method='sbr') # rayshoot_method can be 'sbr' or 'grid'. only monostatic is supported for now, bistatic is a WiP.
...
rcs.num_freqs = 3 # number of frequency samples, this is the number of samples in the frequency domain, using 3 so middle sample is center freq
rcs.bandwidth = 300e6
rcs.polarization='VV'
rcs.num_waves_to_solve_in_parallel=1
dynamic_modeler_update = True # True, show modeler on each update (Slower), False, show modeler after simulation completes (Faster)
rcs.output_path = '../output/'

# rcs output parameters
rcs.incident_wave.phi_start = 0
rcs.incident_wave.phi_stop = 360
...
actor_plane_name = all_actors.add_actor(filename=str(paths.models / 'cessna.stl'),
...
all_actors.actors[actor_cr_name].coord_sys.rot = euler_to_rot(phi=-45-90, order='zyz') # rotation in radians
...
y = radius * np.sin(np.deg2rad(rcs.incident_wave.phi_domain))
