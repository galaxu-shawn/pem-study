...
actor_target_name = all_actors.add_actor(filename=os.path.join(paths.models,'F.stl'),
                                 mat_idx=mat_manager.get_index('pec'),
                                 color='black',
                                 target_ray_spacing=0.001)
...
num_pulse_CPI = nz # static simulation so all pulses we will be the same (no moving targets), simulation setup requires >1 pulse
# ToDo, still need to work on positioning the radar device and the target. No sure I am correctly accounting for
# phase when using a single Tx with multiple Rx.
# This example will create a row of Rx antenna in the z direciton, and only scan(solving) in the y direction. The Rx
# antennas will be placed in a linear array, need to correctly account for phase
...
all_positions = np.zeros((ny, 3))
idx = 0
all_idx = []
idx_z = 0
for idx, y in enumerate(positions_y):
    all_positions[idx][0] = antenna_offset
    all_positions[idx][1] = y
    all_positions[idx][2] = 0
    all_idx.append(idx_z)
    idx_z += 1
all_idx = np.array(all_idx)
all_actors.actors[actor_radar_name].coord_sys.lin = [0, 0, apertureZ]
...
if export_debug:
    debug_camera = DebuggingCamera(hMode=ant_device.modes[which_mode],
...
for idx in tqdm(range(len(all_positions))):
    ...
    imData[idx] = np.angle(response[0, 0, :, 1]) # [tx][all_rx][center chirp][center freq]
    response = response[0, 0, :, 1]# [tx][all_rx][center chirp][center freq]
    all_results[idx] = response
    # calculate response in dB to overlay in pyvista plotter

    if export_debug:
        if idx == 0 or idx == numFrames - 1:
...
fig, ax = plt.subplots()
ax.imshow(np.flipud(np.abs(step3)))
plt.show()
