...
radius_to_capture = 200 #meters, geometry clipped at this radius from center_of_scene
...
#######################################################################################################################
# This is a simple routine to graphically choose the area of a large city that we want to simulate. The point picking
# is not perfect and you might find the end result to be offset from the point you clicked. this is because it chooses
# the closes mesh vertices near where you clicked
if custom_picked_point:
    pl = pv.Plotter()
    outline_mesh = pv.Cube(bounds=[-radius_to_capture,radius_to_capture,
                                   -radius_to_capture,radius_to_capture,
                                   -radius_to_capture,radius_to_capture])
    pl.add_mesh(outline_mesh, color='red', style='wireframe')
    def callback(pos):
        pl.add_text(f'{pos}', name='pos',position='lower_left')
        bounds = [pos[0]-radius_to_capture,pos[0]+radius_to_capture,
                  pos[1]-radius_to_capture,pos[1]+radius_to_capture,
                  pos[2]-terrain_mesh.bounds[4],pos[2]+terrain_mesh.bounds[5]]
        ac = list(pl.actors.keys())[0]
        if hasattr(pl.actors[ac], 'mapper'):
            pl.actors[ac].mapper.dataset = pv.Cube(bounds=bounds)
        # pl.add_mesh(pv.Cube(pos=pos), opacity=1, color='red')
        pl.render()
    # Tracks right clicks
    pl.track_click_position(callback=callback)
    buildings_mesh = pv.read(os.path.join(paths.models,'Paris/buildings.stl'))
    terrain_mesh = pv.read(os.path.join(paths.models,'Paris/terrain2.stl'))
    pl.add_mesh(buildings_mesh, pickable=True,color='grey')
    pl.add_mesh(terrain_mesh, pickable=True,color='green')
    pl.enable_point_picking(show_message='Pick a point (Right Mouse)')
    pl.show()
    selected_point = pl.picked_point
    if selected_point is None:
        selected_point = [0,0,0]
    center_of_scene[0] = selected_point[0]
    center_of_scene[1] = selected_point[1]
area_bounds = [center_of_scene[0]-radius_to_capture,center_of_scene[0]+radius_to_capture,center_of_scene[1]-radius_to_capture,center_of_scene[1]+radius_to_capture]
terrain_name = all_actors.add_actor(name='terrain1',
                                    filename=os.path.join(paths.models,'Paris/Paris.json'),
                                    include_texture=False, map_texture_to_material=False, target_ray_spacing=.05,
                                    clip_at_bounds=area_bounds,center_geometry=center_image)
