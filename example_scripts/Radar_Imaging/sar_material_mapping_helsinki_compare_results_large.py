...
output_size_num_pixels_range = 2056
output_size_num_pixels_cross_range = 2056 # number of pixels in the output image, this is the size of the output image we want.
...
sar.max_num_refl = 1
sar.max_num_trans = 1
sar.ray_density = 0.5 # ['0.001','0.005','.01','0.025','0.05','0.1','0.25','0.5']
...
for idx in tqdm(range(num_of_looks)):
    ...
    to_plot = 10*np.log10(np.fmax(np.abs(image_final_window),1e-10)).T
    ...
    str_to_append = f'{sar.ray_density}_{sar.ray_shoot_method}_{idx:03d}_large_enhanced.npy'
