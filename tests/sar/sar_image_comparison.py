...
ray_density = 0.35 #[0.01,0.05,0.1,0.25,0.35]
shoot_method = 'sbr'
skip_terminal_bnc = False
enhanced_ray_processing=True

num_of_looks = 1
center_freq = 10e9
desired_max_range = 500 # use 2500 for 7x7 tiles, 400 for 2x2 tiles
desired_max_cross_range = 500
resolution = 0.595  # meters

upsample_factor = 1
num_range_bins = upsample_factor*int(desired_max_range/resolution) # number of samples in the range domain
num_cross_range_bins = upsample_factor*int(desired_max_cross_range/resolution) #
...
print(f'aspect: {aspect_ang_phi}')
print(f'num_aspect_angle: {num_aspect_angle}')
...
tiles_x = 2 # using 2x2 for small scene, 7x7 for large
tiles_y =2
center_pos = [0,0,0]
z_max=0
all_meshes = []
...
for x in range(tiles_x):
    for y in range(tiles_y):
        ...
        if not os.path.exists(os.path.join(terrain_path, tile_name + '.vtp')):
            ...
            mesh = pv.read(os.path.join(terrain_path, tile_name + '.vtp'))
            ...
            # center_pos = np.array([x_center, y_center, 0])
...
image_method='fft'##'fft' polar_format isar
...
sar.output_path = os.path.join(paths.output,'helsinki_sar')
sar.go_blockage = -1  # set to -1 if no GO blockage, set to 0 or higher for GO blockage
sar.max_num_refl = 1
sar.max_num_trans = 1
sar.ray_density = ray_density #['0.01','0.05','0.1','0.25','0.5']# ['0.001','0.005','.01','0.025','0.05','0.1','0.25','0.5']
sar.ray_shoot_method = shoot_method  # use grid or sbr method for ray shooting
# radar parameters
...
sar.gpu_device=1
sar.gpu_quota=0.95
sar.enhanced_ray_processing=enhanced_ray_processing
...
for idx in tqdm(range(num_of_looks)):
    ...
    to_plot = 20*np.log10(np.abs(image_final_window)).T
    ...
    if sar.enhanced_ray_processing:
        output_file_name = f'_rd{sar.ray_density}_shoot{sar.ray_shoot_method}_enhanced.npy'
    else:
        output_file_name = f'_rd{sar.ray_density}_shoot{sar.ray_shoot_method}.npy'
    np.save(os.path.join(sar.output_path,'image'+output_file_name),image_final_window)
    np.save(os.path.join(sar.output_path,'time'+output_file_name),sar.sim_performance_time)
...
# Create initial plot using physical domains for axis labeling
# map image pixels to physical cross-range (x) and range (y) coordinates
extent = (crossrange_domain[0], crossrange_domain[-1], range_domain[0], range_domain[-1])
im = ax.imshow(selected_image, cmap='bone_r', vmin=initial_vmin, vmax=initial_vmax, 
               extent=extent, aspect='auto', origin='lower')
ax.set_title(f'Interactive SAR Image -\nAdjust sliders to change dynamic range')
...
# create readable tick marks from the physical domains (choose a small number)
def _select_ticks(domain, n=6):
    if domain is None or len(domain) == 0:
        return []
    return np.linspace(domain[0], domain[-1], num=min(n, len(domain)))

xticks = _select_ticks(crossrange_domain, n=6)
yticks = _select_ticks(range_domain, n=6)
